...
sub  updateRankHashes{
	...
	else
	{
		...
		#my $newRank = getRank($currentDst,\@rankedTargets2,\%currentEdgeTargetsDistance2);
		#$ranks1{$key} = 1 + first_index {$_ == $currentDst} @rankedTargets1;
		#print "Before and after subroutine call:",$ranks1{$key},":",$newRank,"\n";
		...
		#These ranks are with tied positions
		$ranks1{$key} = getRank($currentDst,\@rankedTargets1,\%currentEdgeTargetsDistance1);
		$ranks2{$key} = getRank($currentDst,\@rankedTargets2,\%currentEdgeTargetsDistance2);
		$ranks3{$key} = getRank($currentDst,\@rankedTargets3,\%currentEdgeTargetsDistance3);
		$ranks4{$key} = getRank($currentDst,\@rankedTargets4,\%currentEdgeTargetsDistance4);
		$ranks5{$key} = getRank($currentDst,\@rankedTargets5,\%currentEdgeTargetsDistance5);
		$ranks6{$key} = getRank($currentDst,\@rankedTargets6,\%currentEdgeTargetsDistance6);
		$ranks7{$key} = getRank($currentDst,\@rankedTargets7,\%currentEdgeTargetsDistance7);
		$ranks8{$key} = getRank($currentDst,\@rankedTargets8,\%currentEdgeTargetsDistance8);
		$ranks9{$key} = getRank($currentDst,\@rankedTargets9,\%currentEdgeTargetsDistance9);
...
sub getRank{
	my $dst = shift;
	my $rankedArrayRef = shift;
	my @rankedArray = @{$rankedArrayRef};
	my $distanceHashRef = shift;
	my %distanceHash = %{$distanceHashRef};

	# Ranked array contains key sorted by values
	# Iterate over them, check values, increment rank
	# when a higher value is found
	my $tieRank = 0;
	my $actualRank = 0;
	my $index =  first_index {$_ == $currentDst} @rankedArray;

	my $currentMaxVal = $distanceHash{$rankedArray[0]};
	$tieRank++;
	$actualRank++;
	...
	for(my $i=1; $i<=$index; $i++)
	{
		...
		my $newVal = $distanceHash{$rankedArray[$i]};
		if($newVal > $currentMaxVal)
		{
			# The value increases, both ranks should be increased
			$actualRank++;
			$tieRank = $actualRank;
			$currentMaxVal = $newVal;
		}
		else
		{
			# The new distance value is same as the max encountered till now
			# Tie rank remains same, actual increases
			$actualRank ++;
		}
	}
	# print "Dst:Actual Rank:Tie Rank:",$dst,":",$actualRank,":",$tieRank,"\n";
	return $tieRank;
